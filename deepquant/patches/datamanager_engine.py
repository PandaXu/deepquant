...
from deepquant.trader.engine import BaseEngine, MainEngine, EventEngine
from deepquant.trader.constant import Interval, Exchange
from deepquant.trader.object import BarData, TickData, ContractData, HistoryRequest
from deepquant.trader.database import BaseDatabase, get_database, BarOverview, DB_TZ
from deepquant.trader.datafeed import BaseDatafeed, get_datafeed
    ...
    def sync_minute_data(self, output: Callable = print) -> int:
        ...
        for ov in overviews:
            ...
            try:
                ...
                if count > 0:
                    synced += 1
                    output(f"[DataManager] 同步分钟数据: {ov.symbol}.{ov.exchange.value} → {count}条")
            except Exception as e:
                output(f"[DataManager] 同步失败 {ov.symbol}: {e}")
        return synced
