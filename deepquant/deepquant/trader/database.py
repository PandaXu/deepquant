...
def get_database() -> BaseDatabase:
    ...
    with _db_lock:
        ...
        # vnpy_sqlite.SqliteDatabase.__init__ calls connect() on a
        # module-level peewee object.  Patch that object to make
        # connect() idempotent before creating the first Database().
        import vnpy_sqlite.sqlite_database as sqldb
        _orig_connect = sqldb.db.connect
        def _safe_connect(reuse=[]):
            if sqldb.db.is_connection_usable():
                return True
            return _orig_connect()
        sqldb.db.connect = _safe_connect
