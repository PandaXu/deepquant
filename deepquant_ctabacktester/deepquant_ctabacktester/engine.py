...
from deepquant.trader.engine import BaseEngine, MainEngine
from deepquant.trader.constant import Interval
from deepquant.trader.utility import extract_vt_symbol
from deepquant.trader.object import HistoryRequest, TickData, BarData, ContractData
from deepquant.trader.datafeed import BaseDatafeed, get_datafeed
from deepquant.trader.database import BaseDatabase, get_database
...
class BacktesterEngine(BaseEngine):
    ...
    def _ensure_engine(self) -> None:
        """Lazy init for headless/Web usage (Desktop UI calls init_engine via widget)."""
    ...
    def run_backtesting(
        ...
        try:
            ...
            if class_name not in self.classes:
                self.write_log(_("找不到策略类：{}").format(class_name))
                return
